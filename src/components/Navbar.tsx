...
Vars.addDerivedDefault(vars => ({
  ...
  "navbar-dropdown-boxed-shadow": `0 8px 8px ${rgba(
    vars["black"],
    0.1
  )}, 0 0 0 1px ${rgba(vars["black"], 0.1)}`,
...
const colorClasses = ({ theme }) =>
  Object.entries(theme["colors"]).reduce<TODO>(
    (acc, [name, [color, color_invert]]: TODO) => css`
  ${acc}
  &.is-${name} {
...
> = styled.nav`
  ...
  &.has-shadow {
    box-shadow: ${fromTheme("navbar-box-shadow-size")} ${fromTheme(
  ...
  &.is-fixed-bottom {
    bottom: 0;
    &.has-shadow {
      box-shadow: ${fromTheme("navbar-bottom-box-shadow-size")} ${fromTheme(
  "navbar-box-shadow-color"
)};
    }
  }
  ...
  ${desktop`
    ...
    &.is-spaced {
      padding: ${({ theme }) =>
        `${theme["navbar-padding-vertical"]} ${
          theme["navbar-padding-horizontal"]
        }`};
      ${NavbarStart},
      ${NavbarEnd} {
        align-items: center;
